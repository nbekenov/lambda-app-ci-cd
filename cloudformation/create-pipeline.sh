...
aws cloudformation deploy --stack-name sample-lambda-pipeline --template-file code-pipeline.yaml --parameter-overrides TestAccount=$TestAccount ProductionAccount=$ProdAccount CMKARN=$CMKArn S3Bucket=$S3Bucket RepositoryName=$RepositoryName --capabilities CAPABILITY_NAMED_IAM --profile $TestAccountProfile
echo "====================="
